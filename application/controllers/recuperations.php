...
class Recuperations extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('recuperations_model');
    ...
    public function index($filter = 'requested') {
        $this->auth->check_is_granted('list_recuperations');
        $this->expires_now();
        if ($filter == 'all') {
            $showAll = true;
        } else {
		...
		$data['filter'] = $filter;
        $data['title'] = lang('recup_index_title');
        $data['requests'] = $this->recuperations_model->requests($this->user_id, $showAll);
        ...
        $this->load->view('recuperations/index', $data);
    ...
    public function accept($id) {
        $this->auth->check_is_granted('accept_recuperations');
        $this->load->model('users_model');
        $extra = $this->recuperations_model->get_extra($id);
        if (empty($extra)) {
            show_404();
        }
        ...
        if (($this->user_id != $employee['manager']) && ($this->is_hr == false)) {
            ...
            $this->session->set_flashdata('msg', lang('recup_accept_flash_msg_error'));
            redirect('home');
        } else {
            $this->recuperations_model->accept_extra($id);
            $this->sendMail($id);
            $this->session->set_flashdata('msg', lang('recup_accept_flash_msg_success'));
            if (isset($_GET['source'])) {
                redirect($_GET['source']);
            } else {
                redirect('recuperations');
            }
        }
    }
    ...
    public function reject($id) {
        $this->auth->check_is_granted('reject_recuperations');
        $this->load->model('users_model');
        $extra = $this->recuperations_model->get_extra($id);
        if (empty($extra)) {
            show_404();
        ...
        if (($this->user_id != $employee['manager']) && ($this->is_hr == false)) {
            ...
            $this->session->set_flashdata('msg', lang('recup_reject_flash_msg_error'));
            redirect('home');
        } else {
            $this->recuperations_model->reject_extra($id);
            $this->sendMail($id);
            $this->session->set_flashdata('msg', lang('recup_reject_flash_msg_success'));
            if (isset($_GET['source'])) {
                redirect($_GET['source']);
            } else {
                redirect('recuperations');
    ...
    private function sendMail($id)
    {
        ...
        $message = "";
        if ($extra['status'] == 3) {
            $message = $this->parser->parse('emails/' . $employee['language'] . '/recuperation_accepted', $data, TRUE);
            $this->email->subject(lang('email_recup_request_accept_subject'));
        } else {
            $message = $this->parser->parse('emails/' . $employee['language'] . '/recuperation_rejected', $data, TRUE);
    ...
    public function export($filter = 'requested') {
        ...
        $this->excel->getActiveSheet()->setCellValue('A1', lang('recup_export_thead_id'));
        $this->excel->getActiveSheet()->setCellValue('B1', lang('recup_export_thead_fullname'));
        $this->excel->getActiveSheet()->setCellValue('C1', lang('recup_export_thead_date'));
        $this->excel->getActiveSheet()->setCellValue('D1', lang('recup_export_thead_duration'));
        $this->excel->getActiveSheet()->setCellValue('E1', lang('recup_export_thead_status'));
        ...
        $requests = $this->recuperations_model->requests($this->user_id, $showAll);
        $this->load->model('status_model');
        
        $line = 2;
        foreach ($requests as $request) {
			$statu = $this->status_model->get_label($request['status']);
			switch ($statu){
				case "Requested":
					$statu = "Demandée";
					break;
            ...
            $this->excel->getActiveSheet()->setCellValue('E' . $line, $statu);
            $line++;
        }

        $filename = 'recuperations.xls';
